...
void Player::cameraRender(Map map, SDL_Renderer* renderer, SDL_Texture* texture, int width, int height){
    // int wid = display.w;
    frameBuffer = std::vector<Uint32>(width*height);
    ...
    for(int y=0;y<height/2;y++){
        for(int x=0;x<width;x++){
            ...
            Uint32 color_int = colorInt(pixelColor);
            frameBuffer[y * width + x] = color_int;
    ...
    for(int y=height-1;y>height/2;y--){
        for(int x=0;x<width;x++){
            ...
            double heightVec = 0.66/320;
            Coords rayDir = camX * cameraPlane + playerDirection;
            double p1 =  floorHeight * rayDir.x / (2*(y-height/2) * heightVec);
            double p2 =  floorHeight * rayDir.y / (2*(y-height/2) * heightVec);

            floorHit.x = p1 + playerPos.x;
            floorHit.y = p2 + playerPos.y;

            floorHit = floorHit - floorCoords(floorHit);

            int pixelRow = floor(map.textureSize.x * floorHit.x);
            int pixelCol = floor(map.textureSize.y * floorHit.y);
    ...
    for(int x=0;x<width;x++){
        ...
        int clip_start = drawStart < 0 ? 0 : drawStart;
        int clip_end = drawEnd > height ? height : drawEnd;
        double wallX;
        ...
        for(int y = clip_start; y<clip_end; y++){
